...
class LlamaLoraRouter(nn.Module):
    def __init__(self, hidden_size, experts_num=12, experts_pool_num=8, fixed_experts_num=1, task_experts_num=1, select_experts_num=2, task_num=3):
        super().__init__()
    ...
    def forward(self, hidden_state):
        ...
        # Change here for sentence-level
        # hidden_state_mean = hidden_state.mean(dim=1).unsqueeze(1)
        # hidden_state_expand = hidden_state_mean.expand(-1, seq_length, -1)
        # hidden_state_expand = hidden_state_expand.reshape(-1, hz)

        hidden_state_expand = hidden_state.reshape(-1, hz)

        # TODO
        logits_router = self.router_network(hidden_state_expand)
        ...
        # similarity =  torch.clamp(similarity, min=1.0)
        # rest_cols = (similarity / self.task_experts_num).repeat(1, self.task_experts_num).to(device = indices.device)
        # fixed_score = rest_cols
        ...
        if self.fixed_experts_num != 0:
            fixed_values = fixed_indices.unsqueeze(1).clone().to(device=top_k_indices.device).expand(btz, seq, -1)
            top_k_indices = torch.cat([top_k_indices, fixed_values], dim=-1)
            fixed_score = fixed_score.unsqueeze(1).clone().to(device=top_k_indices.device).expand(btz, seq, -1)
    ...
    def model_replay(self, inputs_embeds):
        ...
        logits_router = self.softmax(logits_router)
...
#         self.lora_router = LlamaLoraRouter(self.hidden_size, experts_num=self.experts_num, experts_pool_num=self.experts_pool_num, fixed_experts_num=self.fixed_experts_num, task_experts_num=self.task_experts_num, select_experts_num=self.select_experts_num, task_num=self.task_num)
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        def agg_lora_states(hidden_states, lora_layer, top_k_indices, top_k_scores, expert_mask, q):
            num_heads = self.num_heads if q else self.num_key_value_heads
            batch_size, sequence_length, hidden_dim = hidden_states.shape
            hidden_states = hidden_states.view(-1, hidden_dim)
            final_hidden_states = torch.zeros((batch_size * sequence_length, num_heads * self.head_dim), dtype=hidden_states.dtype, device=hidden_states.device)
            for expert_idx in range(self.experts_num):
                expert_layer = lora_layer[expert_idx]
                idx, top_x = torch.where(expert_mask[expert_idx])
                top_x_list = top_x.tolist()
                ...
                current_hidden_states = expert_layer(current_state).squeeze() * top_k_scores[top_x_list, idx_list, None]
                final_hidden_states.index_add_(0, top_x, current_hidden_states.to(hidden_states.dtype))
            return final_hidden_states.view(batch_size, sequence_length, num_heads * self.head_dim)

        top_k_indices, top_k_scores, expert_mask = self.lora_router(hidden_states)
        lora_experts_q = self.lora_experts_q
        lora_experts_v = self.lora_experts_v
        query_states = (self.q_proj(hidden_states)+agg_lora_states(hidden_states, lora_experts_q, top_k_indices, top_k_scores, expert_mask, True)).view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        
        value_states = (self.v_proj(hidden_states)+agg_lora_states(hidden_states, lora_experts_v, top_k_indices, top_k_scores, expert_mask, False)).view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def memory_replay(self, inputs_embeds, replay_labels, replay_simi,index):
        kl_loss = None
        T = 10
        if replay_labels is not None and replay_simi is not None:
            kl_loss_fct = nn.KLDivLoss(reduction="batchmean")
            current_logits, similarity = self.model.layers[index].self_attn.lora_router.model_replay(inputs_embeds)
            kl_loss_1 = torch.relu(kl_loss_fct(F.log_softmax(current_logits/T,dim=1), F.softmax(replay_labels/T,dim=1)))
            kl_loss_2 = torch.relu(kl_loss_fct(F.log_softmax(similarity/T,dim=1), F.softmax(replay_simi/T,dim=1)))
